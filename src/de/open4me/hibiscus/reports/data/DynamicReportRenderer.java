...
public final class DynamicReportRenderer
{
    ...
    public RenderedReport render(String template)
    {
        ...
        try
        {
            RenderResult result = jinjava.renderForResult(template,
                Map.of("konten", accounts,
                    "kontogruppen", new ReportAccountGroupsProxy(accounts),
                    "umsaetze", new ReportTransactionsProxy(transactionProvider)));
            List<String> errors = result.getErrors().stream().map(DynamicReportRenderer::format).toList();
