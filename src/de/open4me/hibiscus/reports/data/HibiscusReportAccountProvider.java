    ...
    public List<ReportAccount> loadAccounts(KontoFilter filter) throws Exception
    {
        ...
        for (Konto account : KontoUtil.getKonten(filter))
        {
            ...
            result.add(new ReportAccount(account.getID(), account.getSaldo(), account.getSaldoAvailable(),
                ...
                account.getKontonummer(), account.getKundennummer(), account.getBezeichnung(), account.getIban(),
                account.getKategorie(), account.getAccountType(), account.getBackendClass(),
                !account.hasFlag(Konto.FLAG_DISABLED),
                account.hasFlag(Konto.FLAG_OFFLINE),
